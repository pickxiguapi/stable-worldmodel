...
tmux new-session -d -s lewm-tworoom -c "$PWD" \
  ...
  ./.venv/bin/python scripts/train/lewm.py \
    data=tworoom \
    data.dataset.name=$PWD/data/tworoom.h5 \
...
tmux new-session -d -s lewm-reacher -c "$PWD" \
  ...
  ./.venv/bin/python scripts/train/lewm.py \
    data=dmc \
    data.dataset.name=$PWD/data/reacher.h5 \
...
tmux new-session -d -s lewm-pusht -c "$PWD" \
  ...
  ./.venv/bin/python scripts/train/lewm.py \
    data=pusht \
    data.dataset.name=$PWD/data/pusht_expert_train.h5 \
...
tmux new-session -d -s lewm-ogbench_cube -c "$PWD" \
  ...
  ./.venv/bin/python scripts/train/lewm.py \
    data=ogb \
    data.dataset.name=$PWD/data/cube_single_expert.h5 \
