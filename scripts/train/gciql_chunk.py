...
def get_chunk_td_constants(cfg):
    """Chunked (k-step) TD constants for the k-step value backup.

    With ``frameskip > 1`` each sampled "step" in the clip is actually
    ``frameskip`` raw environment frames apart, and the actor predicts a
    ``frameskip``-length action chunk. The value backup therefore spans

        span = frameskip * td_offset   raw frames

    and the correct bootstrap discount is ``gamma ** span``, while the reward
    term is the discounted sum over the skipped frames:

        R_chunk = sum_{i=0}^{span-1} gamma^i * r(s_{t+i})

    Since the sparse reward is -1 on every non-terminal frame, a fully
    non-terminal chunk accumulates ``-reward_lump``. This follows the flat
    Q-Chunking design in HiQC (arXiv:2607.20834, Eq. 3 & 7).

    Returns:
        gamma_span (float): discount applied to the bootstrapped next value.
        reward_lump (float): sum_{i=0}^{span-1} gamma^i.
    """
    gamma = cfg.get('discount', 0.99)
    span = cfg.frameskip * cfg.dinowm.td_offset
    gamma_span = gamma ** span
    reward_lump = (1 - gamma_span) / (1 - gamma)
...
def get_gciql_critics_model(cfg):
    ...
    def forward(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            # Chunked reward: discounted sum of sparse rewards over the
            # `frameskip * td_offset` frames skipped by the k-step backup.
            # Every non-terminal frame contributes -1, so a fully
            # non-terminal chunk accumulates `-reward_lump`.
            gamma_span, reward_lump = get_chunk_td_constants(cfg)
            reward = -masks * reward_lump
        ...
        with torch.no_grad():
            ...
            # k-step chunk backup (HiQC Eq. 3/7):
            # Q(s,a,g) = R_chunk + γ^span * V(s',g)
            q_target = reward + gamma_span * masks * next_v
...
def get_gciql_actor_model(cfg, trained_critics_model):
    ...
    def forward(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            # Chunk-conditioned advantage (HiQC Eq. 8):
            #   adv(s, a_chunk, g) = Q(s, a_chunk, g) - V(s, g)
            # The critic is conditioned on the full action chunk, so the AWR
            # weight evaluates the specific chunk instead of just the value
            # difference between states.
            actions = batch['action'][:, : cfg.dinowm.history_size]
            q = self.model.critic_predictor(
                embedding_flat, actions, goal_embedding_flat
            )  # (B, T, 1)
            value = self.model.value_predictor(
                embedding_flat, goal_embedding_flat
            )  # (B, T, 1)
            advantage = q - value  # (B, T, 1)
        ...
        losses_dict[f'{prefix}debug/q_mean'] = q.mean().detach()
        losses_dict[f'{prefix}debug/q_min'] = q.min().detach()
        losses_dict[f'{prefix}debug/q_max'] = q.max().detach()
    ...
    gciql_model = swm.wm.gcrl.GCRL(
        ...
        critic_predictor=spt.backbone.EvalOnly(
            trained_critics_model.model.critic_predictor.student
        ),
...
# Main Entry Point
# ============================================================================
@hydra.main(version_base=None, config_path='./config', config_name='gciql_chunk')
def run(cfg):
